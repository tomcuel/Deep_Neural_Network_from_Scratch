...
class DeepNeuralNetwork:
    def __init__(
        ...
        self.activations = activations or ["relu"] * (len(layer_dims) - 2) + ["softmax"]
        self.lr = lr
        self.epochs = epochs
        self.batch_size = batch_size
        self.loss_function = loss_function
        self.optimizer_type = optimizer_type
        self.dropout_rates = dropout_rates or [0.0] * (len(layer_dims) - 1)
    ...
    def evaluate_classification(
        ...
        """
        Return dictionary with common classification metrics
        ...
        y_prob : Optional[np.ndarray]
            Predicted probabilities for positive class (for ROC AUC and Brier Score)
            
        Returns:
        dict
            Dictionary containing accuracy, precision, recall, F1-score, ROC AUC, and Brier Score
        """
        results = {
        ...
        if y_prob is not None:
            results["roc_auc"] = self.roc_auc_score(y_true, y_prob)
            results["brier_score"] = self.brier_score_loss(y_true, y_prob)
        return results