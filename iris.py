...
import argparse
import os
os.makedirs("model_configs/iris", exist_ok=True)
...
# 1. Argument Parsing
# =======================================
parser = argparse.ArgumentParser(description='Iris Dataset Classification with DNN from Scratch')
...
parser.add_argument('-t', '--trials', type=int, default=50, help='Number of Optuna trials for hyperparameter optimization')

# User-defined hyperparameters (used if Optuna is NOT enabled), can be one by one specifically default values otherwise
parser.add_argument('-l', '--layer_dims', nargs='+', type=int, default=[10, 10])
...
parser.add_argument('-d', '--dropout_rates', nargs='+', type=float, default=[0.1, 0.1])
...
# =======================================
# save test metrics
with open("model_configs/iris/metrics.yaml", "w") as file:
    results = {
...
with open("model_configs/iris/model_layout.yaml", "w") as file:
    config = {
        "layer_dims": layer_dims,
        "activations": activations,
        "learning_rate": lr,
